...
def over_thresh(
    ...
    color_thresh1 : int, optional
        Threshold for color 1., by default COLOR_THRESH1
    color_thresh2 : int, optional
        Threshold for color 2., by default COLOR_THRESH2
    ...
    for i in range(length):
        if not (
            (color1[i].sum() <= color_thresh1) or (color2[i].sum() <= color_thresh2)
        ):
            result1.append(color1[i])
            result2.append(color2)
